...
def train(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            imgs = imgs.to(device, non_blocking=True).float() / 16383.0  # uint8 to float32, 0-255 to 0.0-1.0
...
if __name__ == '__main__':
    main_path = "/truba/home/taksoy/workspace/yolov4-reasoner"
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', type=str, default='yolov4-p5.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default=main_path+'/models/yolov4-p6-reasoner.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=main_path+'/data/coco.yaml', help='data.yaml path')
    # parser.add_argument('--hyp', type=str, default='', help='hyperparameters path, i.e. data/hyp.scratch.yaml')
    parser.add_argument('--hyp', type=str, default=main_path+'/data/hyp.scratch.yaml', help='hyperparameters path, i.e. data/hyp.scratch.yaml')
    parser.add_argument('--epochs', type=int, default=451)
    ...
    parser.add_argument('--logdir', type=str, default=main_path+'/runs/', help='logging directory')
    opt = parser.parse_args()

    # opt.resume = 'get_last'
    opt.weights = False
    opt.resume = main_path+'/runs/exp2/weights/last.pt'

    # Resume
    if opt.resume:
        last = get_latest_run(search_dir=main_path+'/runs') if opt.resume == 'get_last' else opt.resume  # resume from most recent run
    ...
    opt.hyp = main_path+'/data/hyp.scratch.yaml'
