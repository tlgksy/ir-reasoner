...
def create_modules(module_defs, img_size, cfg):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        elif mdef['type'] == 'reasoning':
            emb_size = int(mdef['emb_size'])
            num_heads = int(mdef['num_heads'])
            depth = int(mdef['depth'])
            expansion_rate = int(mdef['expansion_rate'])
            # Define reasoning layer
            reasoning_layer = ReasoningLayer(emb_size, num_heads, depth, expansion_rate)
            modules.add_module('reasoning', reasoning_layer)
